...
<<<<<<< HEAD
  Volume2, VolumeX, User, ChevronLeft, LogIn, Plus, Edit3, AtSign,
  Hash, Sun, Moon, Copy, Reply, Heart, Trash2, Star,
  MessageSquare, Users, Zap, Sparkles, Music, Radio,
  Waves, Headphones
=======
...
<<<<<<< HEAD
  ...
  { name:"Deep purple",     css:"radial-gradient(ellipse at 30% 60%,#1e0d3a 0%,#0d0921 50%,#050310 100%)" },
  { name:"Midnight blue",   css:"radial-gradient(ellipse at 60% 20%,#001a3a 0%,#000d22 60%,#000510 100%)" },
];
...
class SoundEngine {
  ...
  private play(freq: number, type: OscillatorType, dur: number, vol = 0.12, delay = 0) {
  ...
  sendMessage() {
    this.play(880, "sine", 0.08, 0.1);
    this.play(1100, "sine", 0.06, 0.06, 0.08);
  }

  receiveMessage() {
    this.play(660, "sine", 0.1, 0.1);
    this.play(880, "sine", 0.08, 0.07, 0.1);
    this.play(1100, "triangle", 0.05, 0.05, 0.2);
  }

  notification() {
    this.play(523, "sine", 0.12, 0.12);
    this.play(659, "sine", 0.1, 0.1, 0.1);
    this.play(784, "sine", 0.08, 0.08, 0.2);
  }

  join() {
    [261, 329, 392, 523].forEach((f, i) => this.play(f, "triangle", 0.15, 0.08, i * 0.07));
  }

  error() {
    this.play(200, "sawtooth", 0.15, 0.1);
    this.play(150, "sawtooth", 0.1, 0.08, 0.12);
  }
}
...
/* ═══════════ ANIMATED BACKGROUND ═══════════ */
const AnimatedBg = ({ theme }: { theme: string }) => {
  const colors = {
    dark:  ["#6366f1", "#8b5cf6", "#3b82f6"],
    cyber: ["#00f5ff", "#0ea5e9", "#06b6d4"],
    ocean: ["#818cf8", "#6366f1", "#4f46e5"],
  };
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      {[0, 1, 2].map(i => (
        <motion.div key={i}
          ...
          style={{ background: c[i], width: 300 + i * 100, height: 300 + i * 100 }}
          animate={{
            x: [i * 200, i * 200 + 80, i * 200 - 40, i * 200],
            y: [i * 150, i * 150 - 60, i * 150 + 40, i * 150],
          }}
          transition={{ duration: 15 + i * 5, repeat: Infinity, ease: "easeInOut" }}
        />
      ))}
      {/* Scanline effect for cyber */}
      {theme === "cyber" && (
        <div className="absolute inset-0 opacity-[0.02]"
          style={{ backgroundImage: "repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(0,245,255,0.3) 2px, rgba(0,245,255,0.3) 4px)" }} />
      )}
      {/* Grid for ocean */}
      {theme === "ocean" && (
...
const Particles = ({ color }: { color: string }) => {
  const particles = Array.from({ length: 12 }, (_, i) => ({
    ...
    size: Math.random() * 3 + 1, delay: Math.random() * 5, dur: Math.random() * 10 + 8,
  }));
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {particles.map(p => (
        <motion.div key={p.id} className="absolute rounded-full opacity-20"
...
<<<<<<< HEAD
  const go=()=>{ const prof=loadProf(); if(p===prof.appLock){localStorage.setItem(LOCK_KEY,"0");onUnlock();}else{soundEngine.error();setErr("Noto'g'ri parol");setP("");} };
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center gap-8" style={{background:"#0f1117"}}>
      <AnimatedBg theme="dark" />
      <Particles color={color} />
      <motion.div initial={{scale:0.5,opacity:0}} animate={{scale:1,opacity:1}} transition={{type:"spring",damping:15}}
        className="h-28 w-28 rounded-3xl flex items-center justify-center shadow-2xl relative z-10" style={{background:color}}>
        <Lock className="h-14 w-14 text-white"/>
        <div className="absolute inset-0 rounded-3xl" style={{boxShadow:`0 0 40px ${color}88`}}/>
      </motion.div>
      <motion.div initial={{y:20,opacity:0}} animate={{y:0,opacity:1}} transition={{delay:0.1}} className="text-center relative z-10">
      ...
      <motion.div initial={{y:20,opacity:0}} animate={{y:0,opacity:1}} transition={{delay:0.2}} className="w-72 space-y-3 relative z-10">
=======
...
const EmojiPicker = ({onPick,onClose,T}:{onPick:(e:string)=>void;onClose:()=>void;T:Record<string,string>}) => (
  <motion.div initial={{opacity:0,y:12,scale:0.92}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:12,scale:0.92}}
    ...
    <div className="grid grid-cols-8 gap-0.5 p-3 max-h-56 overflow-y-auto">
      {EMOJIS.map((e,i)=>(
<<<<<<< HEAD
        <motion.button key={i} whileHover={{scale:1.3}} whileTap={{scale:0.9}} onClick={()=>{onPick(e);onClose();soundEngine.sendMessage();}}
=======
...
<<<<<<< HEAD
/* ═══════════ SOUND VISUALIZER ═══════════ */
const SoundBar = ({ active, color }: { active: boolean; color: string }) => (
  <div className="flex items-center gap-[2px] h-4">
    {[0,1,2,3,4].map(i => (
      <motion.div key={i}
        className="w-[3px] rounded-full"
        style={{ background: active ? color : "rgba(255,255,255,0.2)" }}
        animate={active ? { height: ["4px", "14px", "6px", "10px", "4px"] } : { height: "4px" }}
...
<<<<<<< HEAD
  const br_tl = isMine ? "20px" : (nextSame?"20px":"4px");
...
<<<<<<< HEAD
                className="h-9 w-9 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-lg"
                style={{background:`linear-gradient(135deg,${msg.senderColor||autoColor(msg.sender)},${msg.senderColor||autoColor(msg.sender)}88)`}}>
=======
...
>>>>>>> 9e3f45b0ee34d0c9b3b0b17417f24ac9ba002cd8
      <div className={`flex flex-col ${isMine?"items-end":"items-start"} max-w-[68%] sm:max-w-[55%]`}>
        ...
        {msg.type==="sticker" ? (
<<<<<<< HEAD
          <motion.div whileHover={{scale:1.15,rotate:5}} className="text-5xl select-none py-1 cursor-pointer"
            onClick={()=>setReacted(r=>!r)}>
            {msg.text}
            <AnimatePresence>
              {reacted && <motion.span initial={{scale:0,y:0}} animate={{scale:1,y:-20}} exit={{scale:0,opacity:0}}
                className="absolute text-xs">❤️</motion.span>}
            </AnimatePresence>
          </motion.div>
=======
...
<<<<<<< HEAD
            {isMine && (
              <div className="absolute inset-0 rounded-[18px] blur-md opacity-20 -z-10" style={{background:T.accent}}/>
            )}
            <div style={{
              background: isMine ? `linear-gradient(135deg,${T.accent},${T.accent}cc)` : T.bubble_other,
              borderRadius: `${br_tl} ${br_tr} 20px 20px`,
=======
      ...
      <AnimatePresence>
        {hover && msg.type==="text" && (
          <motion.div initial={{opacity:0,scale:0.8,x:isMine?8:-8}} animate={{opacity:1,scale:1,x:0}} exit={{opacity:0,scale:0.8}}
            ...
            {[
<<<<<<< HEAD
              {icon:Reply,  fn:()=>onReply(msg)},
              {icon:Copy,   fn:()=>onCopy(msg.text)},
              {icon:Heart,  fn:()=>setReacted(r=>!r)},
            ].map((a,i)=>(
...
<<<<<<< HEAD
  const save=()=>{ onSave(p); onClose(); soundEngine.notification(); };

  const TABS=[{id:"profile",label:"Profil",icon:User},{id:"appearance",label:"Ko'rinish",icon:Sparkles},{id:"privacy",label:"Maxfiylik",icon:Shield},{id:"notif",label:"Ovoz & Bildirishnoma",icon:Bell}] as const;
=======
...
<<<<<<< HEAD
      <AnimatedBg theme={p.theme} />

      <div className="flex items-center gap-3 px-5 py-4 border-b shrink-0 relative z-10" style={{background:T.sidebar,borderColor:T.border}}>
=======
...
<<<<<<< HEAD
      <div className="flex border-b shrink-0 px-2 relative z-10" style={{background:T.sidebar,borderColor:T.border}}>
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id as any)}
            className="flex items-center gap-1.5 px-3 py-3.5 text-xs font-semibold whitespace-nowrap border-b-2 transition-all"
=======
      {/* Tabs */}
          ...
          <div className="rounded-2xl p-5 flex items-center gap-4 border" style={{background:T.card,borderColor:T.border}}>
            <div className="relative">
              <motion.div whileHover={{scale:1.05}} className="h-16 w-16 rounded-2xl flex items-center justify-center text-2xl font-bold text-white shadow-xl"
                style={{background:`linear-gradient(135deg,${p.color},${p.color}88)`,boxShadow:`0 0 20px ${p.color}44`}}>
=======
...
<<<<<<< HEAD
                  className="w-full rounded-xl px-4 py-3 text-sm text-white outline-none resize-none border transition-all"
=======
...
<<<<<<< HEAD
                  className="rounded-xl p-3 border-2 transition-all text-center overflow-hidden relative"
                  style={{background:THEMES[th].bg,borderColor:p.theme===th?T.accent:T.border}}>
                  <div className="h-6 w-full rounded-lg mb-2" style={{background:`linear-gradient(135deg,${THEMES[th].accent},${THEMES[th].accent}66)`}}/>
=======
...
<<<<<<< HEAD
        {tab==="notif" && <>
          <div className="space-y-3">
            {/* Sound toggle */}
            <div className="rounded-2xl border overflow-hidden divide-y" style={{borderColor:T.border,background:T.card}}>
              <div className="flex items-center justify-between px-5 py-4">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-xl flex items-center justify-center" style={{background:`${T.accent}22`}}>
                    {p.sound ? <Volume2 className="h-5 w-5" style={{color:T.accent}}/> : <VolumeX className="h-5 w-5" style={{color:T.muted}}/>}
                  </div>
                  <div>
                    <p className="font-semibold text-white text-sm">Ovoz effektlari</p>
                    <p className="text-xs" style={{color:T.muted}}>Xabar yuborish/olish ovozi</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <SoundBar active={p.sound} color={T.accent} />
                  <motion.button whileTap={{scale:0.9}} onClick={()=>{setP(x=>({...x,sound:!x.sound}));soundEngine.setEnabled(!p.sound);if(!p.sound)soundEngine.notification();}}
                    className="relative h-7 rounded-full transition-all" style={{background:p.sound?T.accent:"rgba(255,255,255,0.1)",width:52}}>
            ...
            <div className="rounded-2xl border p-4 space-y-3" style={{borderColor:T.border,background:T.card}}>
              <p className="text-xs font-bold uppercase tracking-wider" style={{color:T.muted}}>Ovoz namunalari</p>
              <div className="grid grid-cols-2 gap-2">
                {[
                  {label:"Yuborish", fn:()=>soundEngine.sendMessage(), icon:Send},
                  {label:"Qabul",    fn:()=>soundEngine.receiveMessage(), icon:MessageSquare},
                  {label:"Bildirishnoma", fn:()=>soundEngine.notification(), icon:Bell},
                  {label:"Qo'shilish",   fn:()=>soundEngine.join(), icon:Users},
                ].map(s=>(
                  <motion.button key={s.label} whileHover={{scale:1.03}} whileTap={{scale:0.97}}
                    onClick={()=>{soundEngine.setEnabled(true);s.fn();soundEngine.setEnabled(p.sound);}}
                    className="flex items-center gap-2 rounded-xl px-3 py-2.5 text-xs font-semibold border transition"
                    style={{background:"rgba(255,255,255,0.04)",borderColor:T.border,color:T.text}}>
                    <s.icon className="h-3.5 w-3.5" style={{color:T.accent}}/> {s.label}
                  </motion.button>
                ))}
              </div>
            </div>
            ...
            <motion.button whileHover={{scale:1.02}} whileTap={{scale:0.98}} onClick={()=>Notification.requestPermission()}
              ...
              style={{background:`linear-gradient(135deg,${T.accent},${T.accent}aa)`}}>
              <Bell className="h-4 w-4"/> Push bildirishnomaga ruxsat
            </motion.button>
          </div>
=======
        {/* NOTIFICATIONS */}
...
>>>>>>> 9e3f45b0ee34d0c9b3b0b17417f24ac9ba002cd8
  ...
  useEffect(()=>{
<<<<<<< HEAD
    soundEngine.setEnabled(prof.sound && soundOn);
  },[prof.sound, soundOn]);
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      setAuthed(!!session);
      if (session) soundEngine.join();
    };
    
    checkSession();

    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      console.log("Chat auth change:", event, !!session);
      setAuthed(!!session);
      if (session && event === 'SIGNED_IN') soundEngine.join();
    });
...
>>>>>>> 9e3f45b0ee34d0c9b3b0b17417f24ac9ba002cd8
  ...
  useEffect(()=>{
<<<<<<< HEAD
    if(messages.length > prevMsgLen.current && prevMsgLen.current > 0) {
      const last = messages[messages.length-1];
      if(last.sender !== prof.uid) {
        soundEngine.receiveMessage();
        setNewMsgFlash(true);
        setTimeout(()=>setNewMsgFlash(false), 500);
        setTyping(true);
...
<<<<<<< HEAD
  const switchRoom=(id:string)=>{ setRoomId(id); setMessages([]); setShowSide(false); soundEngine.join(); };
  const copyText=(t:string)=>{ navigator.clipboard.writeText(t); setCopied(true); soundEngine.notification(); setTimeout(()=>setCopied(false),2000); };
=======
...
<<<<<<< HEAD
    <div className="h-screen flex items-center justify-center relative overflow-hidden" style={{background:T.bg}}>
      <AnimatedBg theme={prof.theme} />
      <motion.div animate={{rotate:360}} transition={{duration:1,repeat:Infinity,ease:"linear"}} className="h-10 w-10 rounded-full border-2 border-t-transparent relative z-10" style={{borderColor:T.accent}}/>
=======
...
>>>>>>> 9e3f45b0ee34d0c9b3b0b17417f24ac9ba002cd8
  ...
  if(!authed) return (
<<<<<<< HEAD
    <div className="h-screen flex flex-col items-center justify-center gap-8 p-6 relative overflow-hidden" style={{background:T.bg}}>
      <AnimatedBg theme={prof.theme} />
      ...
      <motion.div initial={{scale:0.5,opacity:0}} animate={{scale:1,opacity:1}} transition={{type:"spring",damping:12}}
        className="h-28 w-28 rounded-3xl flex items-center justify-center text-6xl shadow-2xl relative z-10"
        style={{background:`linear-gradient(135deg,${T.accent},${T.accent}66)`,boxShadow:`0 20px 60px ${T.accent}44`}}>💬</motion.div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 w-full max-w-sm relative z-10">
        <motion.button whileHover={{scale:1.03}} whileTap={{scale:0.97}} onClick={()=>navigate("/auth?redirect=/chat")}
          className="flex-1 h-14 rounded-2xl font-bold text-white flex items-center justify-center gap-2 text-base"
          style={{background:`linear-gradient(135deg,${T.accent},${T.accent}88)`,boxShadow:`0 8px 24px ${T.accent}44`}}>
=======
...
>>>>>>> 9e3f45b0ee34d0c9b3b0b17417f24ac9ba002cd8
  ...
  return (
<<<<<<< HEAD
    <div className="h-screen flex overflow-hidden relative" style={{background:T.bg,fontFamily:"'DM Sans',sans-serif"}}>
      <AnimatedBg theme={prof.theme} />
=======
...
<<<<<<< HEAD
                      style={{background:`linear-gradient(135deg,${prof.color},${prof.color}88)`,boxShadow:`0 4px 12px ${prof.color}44`}}>
=======
...
>>>>>>> 9e3f45b0ee34d0c9b3b0b17417f24ac9ba002cd8
        ...
        {/* ── TOPBAR ── */}
<<<<<<< HEAD
        <div className="flex items-center gap-2 px-4 py-3 shrink-0 border-b" style={{background:`${T.sidebar}ee`,borderColor:T.border,backdropFilter:"blur(20px)"}}>
          {/* Back to home */}
...
<<<<<<< HEAD
          <div className="flex items-center gap-1 shrink-0">
            ...
            <motion.button whileHover={{scale:1.1}} whileTap={{scale:0.9}}
              onClick={()=>{ const nv=!soundOn; setSoundOn(nv); soundEngine.setEnabled(nv && prof.sound); if(nv&&prof.sound) soundEngine.notification(); }}
              className="h-9 w-9 rounded-xl flex items-center justify-center transition border"
              style={soundOn?{background:T.card,borderColor:T.accent,color:T.accent}:{background:T.card,borderColor:T.border,color:T.muted}}
...
>>>>>>> 9e3f45b0ee34d0c9b3b0b17417f24ac9ba002cd8
        ...
        {/* ── MESSAGES ── */}
<<<<<<< HEAD
        <div className={`flex-1 overflow-y-auto py-4 relative transition-all duration-300 ${newMsgFlash?"ring-1":"ring-0"}`}
          style={{background:wallBg, '--tw-ring-color': T.accent} as any}>

          {/* Noise overlay */}
          <div className="absolute inset-0 pointer-events-none opacity-[0.025]"
            style={{backgroundImage:"url(\"data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noise'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noise)'/%3E%3C/svg%3E\")"}}/>

          {/* Floating particles on the message area */}
          {(prof.theme === "cyber" || prof.theme === "ocean") && (
            <Particles color={T.accent} />
          )}
          ...
          {messages.length===0 && (
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="flex flex-col items-center justify-center h-full gap-5 text-center px-6">
              <motion.div
                ...
                className="h-24 w-24 rounded-3xl flex items-center justify-center text-5xl shadow-2xl"
...
>>>>>>> 9e3f45b0ee34d0c9b3b0b17417f24ac9ba002cd8
        ...
        {/* ── INPUT BAR ── */}
<<<<<<< HEAD
        <div className="px-3 py-2.5 shrink-0 border-t relative" style={{background:`${T.sidebar}ee`,borderColor:T.border,backdropFilter:"blur(20px)"}}>
=======
...
<<<<<<< HEAD
          ...
          <div className="flex gap-1.5 mb-2 overflow-x-auto scrollbar-hide">
            {["👋","🔥","💬","⚡","🎉","✅","💯","🙏","😂","❤️","👍","🤝"].map(s=>(
              <motion.button key={s} whileHover={{scale:1.25,y:-2}} whileTap={{scale:0.8}}
                onClick={()=>sendMsg(s,"sticker")}
                className="shrink-0 text-xl h-8 w-8 rounded-xl flex items-center justify-center transition"
          ...
          <div className="flex items-end gap-2">
            ...
            <div className="flex gap-1 shrink-0">
              <motion.button whileHover={{scale:1.1}} whileTap={{scale:0.9}} onClick={()=>imgRef.current?.click()}
                ...
                style={{background:T.card,borderColor:T.border,color:T.muted}} title="Fayl yuklash">
                <Paperclip className="h-4 w-4"/>
              </motion.button>
              <motion.button whileHover={{scale:1.1}} whileTap={{scale:0.9}}
                onClick={()=>{const msgs=["Salom! 👋","Usta topsam bo'ladimi?","Narx qancha?","Hozir borishingiz mumkinmi?","Raxmat! 🙏"];sendMsg(msgs[Math.floor(Math.random()*msgs.length)]);}}
                className="h-10 w-10 rounded-xl flex items-center justify-center border transition"
                style={{background:T.card,borderColor:T.border,color:T.muted}} title="Tezkor xabar">
            ...
            <div className="flex-1 flex items-end rounded-2xl border overflow-hidden transition-all"
              ...
              <input ref={inputRef} value={input} onChange={e=>setInput(e.target.value)}
                ...
                className="flex-1 bg-transparent text-sm outline-none px-3.5 py-3"
                style={{fontSize:prof.fontSize,fontFamily:"'DM Sans',sans-serif",color:T.text}}/>
              <motion.button whileHover={{scale:1.1}} whileTap={{scale:0.9}} onClick={()=>setShowEmoji(s=>!s)}
                className="px-2.5 self-center shrink-0 transition"
=======
...
<<<<<<< HEAD
                  className="h-10 w-10 shrink-0 rounded-2xl flex items-center justify-center shadow-xl transition"
                  style={{background:`linear-gradient(135deg,${T.accent},${T.accent}aa)`,boxShadow:`0 6px 20px ${T.accent}55`}}>
                  {sending
                    ?<motion.div animate={{rotate:360}} transition={{duration:0.8,repeat:Infinity,ease:"linear"}}
                      className="h-4 w-4 rounded-full border-2 border-white/30 border-t-white"/>
                    :<Send className="h-4 w-4 text-white" style={{transform:"translateX(1px)"}}/>}
=======
...
<<<<<<< HEAD
                  className="h-10 w-10 shrink-0 rounded-2xl flex items-center justify-center shadow-xl transition-all"
                  style={recording
                    ?{background:"linear-gradient(135deg,#ef4444,#dc2626)",boxShadow:"0 6px 20px rgba(239,68,68,0.5)"}
                    :{background:T.card,border:`1px solid ${T.border}`,color:T.muted}}>
                  {recording
                    ?<motion.div animate={{scale:[1,1.2,1]}} transition={{repeat:Infinity,duration:0.8}}><MicOff className="h-4 w-4 text-white"/></motion.div>
...
<<<<<<< HEAD
          ...
          <AnimatePresence>
            {recording && (
              <motion.div initial={{height:0,opacity:0}} animate={{height:"auto",opacity:1}} exit={{height:0,opacity:0}}
                className="mt-2 flex items-center gap-3 px-3 py-2 rounded-xl overflow-hidden"
                ...
                <span className="text-xs font-bold text-red-400">Yozilmoqda...</span>
                <motion.div animate={{opacity:[1,0,1]}} transition={{duration:1,repeat:Infinity}}
                  className="h-2 w-2 rounded-full bg-red-500 ml-auto"/>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="flex items-center justify-between mt-1.5 px-1">
            <div className="flex items-center gap-1.5">
              ...
              <p className="text-[10px]" style={{color:T.muted}}>
                <span style={{color:prof.color,fontWeight:700}}>@{prof.nick}</span> · Firebase🔥 · {online?"Ulangan":"Uzilgan"}
              </p>
            </div>
            <div className="flex items-center gap-1.5">
              <SoundBar active={soundOn && prof.sound} color={T.accent}/>
              <span className="text-[10px]" style={{color:T.muted}}>{messages.length} xabar</span>
            </div>
=======
          {/* Status bar */}
