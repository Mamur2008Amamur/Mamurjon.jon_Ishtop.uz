...
<<<<<<< HEAD
      ...
      if (user?.id) requestPayload.user_id = user.id;

      let helpRequest: any = null;
      let dbError: any = null;

      const { data: createdRequest, error: helpError } = await supabase
        .from("help_requests")
        .insert(requestPayload)
        .select()
        .single();

      if (helpError) dbError = helpError;
      else helpRequest = createdRequest;
=======
...
<<<<<<< HEAD
      const { error: telegramError } = await supabase.functions.invoke("send-telegram", {
        body: {
          type: "help",
          name: user?.user_metadata?.full_name || user?.email || "Sayt foydalanuvchisi",
          email: user?.email || "",
      ...
      if (dbError && telegramError) throw dbError;
