...
<<<<<<< HEAD
import { Search, ArrowRight, Sparkles, MapPin, Users, Star, CheckCircle, Zap, Shield, TrendingUp, X, Play, Award, Wifi } from "lucide-react";
...
<<<<<<< HEAD
  ...
  { value: "98%",     label: "Mamnun",            icon: TrendingUp },
];

const phoneCards = [
  { name:"Abdulloh K.", role:"Santexnik",  rating:"5.0", price:"80 000", online:true,  color:"#3b82f6", emoji:"🔧",
    img:"https://images.unsplash.com/photo-1560250097-0b93528c311a?w=80&q=80&fit=crop&crop=faces" },
  { name:"Sherzod M.",  role:"Elektrik",   rating:"4.9", price:"70 000", online:true,  color:"#f59e0b", emoji:"⚡",
    img:"https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=80&q=80&fit=crop&crop=faces" },
  { name:"Nilufar X.",  role:"Tozalovchi", rating:"5.0", price:"50 000", online:false, color:"#10b981", emoji:"🧹",
    img:"https://images.unsplash.com/photo-1494790108755-2616b612b7fd?w=80&q=80&fit=crop&crop=faces" },
];

/* ── Compact Professional Phone ── */
const PhoneMockup = () => {
  const [activeCard, setActiveCard] = useState(0);
  const [searchText, setSearchText] = useState("");
  const [tab, setTab] = useState(0);
  const fullTexts = ["Santexnik topish...", "Elektrik usta...", "IT xizmat..."];
  const phaseRef = useRef<"type"|"pause"|"delete">("type");
  const ciRef = useRef(0);
  const iRef = useRef(0);
  const tiRef = useRef<any>(0);

  useEffect(()=>{
    const tick = ()=>{
      const target = fullTexts[iRef.current % fullTexts.length];
      if(phaseRef.current==="type"){
        ciRef.current++; setSearchText(target.slice(0,ciRef.current));
        if(ciRef.current>=target.length){
          phaseRef.current="pause";
          clearInterval(tiRef.current);
          tiRef.current = setTimeout(()=>{ phaseRef.current="delete"; tiRef.current=setInterval(tick,55); },1200);
          return;
        }
      } else if(phaseRef.current==="delete"){
        ciRef.current--; setSearchText(target.slice(0,ciRef.current));
        if(ciRef.current<=0){ iRef.current++; phaseRef.current="type"; }
      }
    };
    tiRef.current = setInterval(tick, 75);
    return ()=>{ clearInterval(tiRef.current); clearTimeout(tiRef.current); };
  },[]);

  useEffect(()=>{
    const t = setInterval(()=>setActiveCard(c=>(c+1)%phoneCards.length), 2600);
  ...
  return (
    <div className="relative select-none" style={{width:260}}>
      {/* Ambient glow */}
      <div className="absolute -inset-4 -z-10 rounded-[50px] opacity-50 blur-3xl"
        style={{background:"radial-gradient(circle, rgba(99,102,241,0.3) 0%, rgba(139,92,246,0.15) 60%, transparent 80%)"}}/>
      ...
      <motion.div animate={{y:[0,-6,0]}} transition={{duration:4.5,repeat:Infinity,ease:"easeInOut"}}
        ...
        style={{
          ...
          background:"linear-gradient(160deg,#1c1f2e 0%,#0f1117 100%)",
          border:"1.5px solid rgba(255,255,255,0.09)",
          boxShadow:"0 30px 70px rgba(0,0,0,0.55), 0 0 0 1px rgba(255,255,255,0.03), inset 0 1px 0 rgba(255,255,255,0.06)"
        }}>
        {/* Glass rim highlight */}
        <div className="absolute inset-0 pointer-events-none z-20" style={{borderRadius:40,
        ...
        <div className="flex items-center justify-between px-6 pt-4 pb-1">
          ...
          <div className="h-[14px] w-[70px] rounded-full bg-black/80"/>
          <div className="flex items-center gap-1">
            <Wifi className="h-2.5 w-2.5 text-white/35"/>
            <div className="flex gap-px items-end">
              {[3,4,6,8].map((h,i)=>(
                <div key={i} className="w-[2.5px] rounded-sm" style={{height:h,background:i<3?"rgba(255,255,255,0.6)":"rgba(255,255,255,0.18)"}}/>
              ))}
            </div>
          </div>
        </div>
        ...
        {/* App header */}
        <div className="px-4 pt-1.5 pb-3 flex items-center justify-between"
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-white font-bold text-[12px] leading-none">IshTop.uz</p>
              <p className="text-white/55 text-[9px] mt-0.5">12 500+ usta online</p>
            </div>
          </div>
          <div className="flex gap-1.5">
            <div className="relative h-6 w-6 rounded-full bg-white/15 flex items-center justify-center text-[11px]">
              🔔
              <div className="absolute -top-0.5 -right-0.5 h-3 w-3 rounded-full bg-red-500 flex items-center justify-center text-[7px] font-bold text-white">3</div>
            </div>
          </div>
        </div>
        ...
        <div className="px-3.5 pt-3 pb-2">
          <div className="flex items-center gap-2 rounded-xl px-3 py-2.5"
            ...
            <div className="flex-1 text-[11px]" style={{color:"rgba(255,255,255,0.8)"}}>
              {searchText||<span style={{color:"rgba(255,255,255,0.2)"}}>Xizmat qidiring...</span>}
              <motion.span animate={{opacity:[1,0,1]}} transition={{duration:0.7,repeat:Infinity}}
                className="inline-block w-0.5 h-3 bg-indigo-400 ml-px align-middle"/>
            </div>
          </div>
        </div>

        {/* Category pills */}
        <div className="flex gap-1 px-3.5 pb-2 overflow-x-hidden">
          {["Barchasi","🔧","⚡","💻"].map((cat,i)=>(
            <div key={i} onClick={()=>setTab(i)} className="shrink-0 rounded-full px-2.5 py-1 text-[9px] font-semibold cursor-pointer"
              style={tab===i
                ?{background:"linear-gradient(135deg,#4338ca,#6d28d9)",color:"white"}
        ...
        <div className="px-3.5 pb-1.5 flex items-center justify-between">
          <div className="flex items-center gap-1">
            ...
            <span className="text-[10px] font-bold text-white/80">Yaqin ustalar</span>
          </div>
          <span className="text-[9px] text-indigo-400 font-medium">Ko'rish →</span>
        </div>

        {/* Specialist cards — 3 compact */}
        <div className="px-3.5 pb-2.5 space-y-1.5">
          {phoneCards.map((c,i)=>(
            <motion.div key={c.name}
              initial={{opacity:0,x:12}} animate={{opacity:1,x:0}} transition={{delay:0.25+i*0.1}}
              onClick={()=>setActiveCard(i)}
              className="flex items-center gap-2 rounded-xl p-2 cursor-pointer"
              style={{
                background:activeCard===i?`${c.color}18`:"rgba(255,255,255,0.04)",
                border:activeCard===i?`1px solid ${c.color}45`:"1px solid rgba(255,255,255,0.05)",
                transition:"all 0.3s"
              }}>
              <div className="relative shrink-0">
                <img src={c.img} alt={c.name} className="h-9 w-9 rounded-lg object-cover"
                  onError={e=>{(e.target as HTMLImageElement).src=`https://placehold.co/36x36/4338ca/white?text=${c.emoji}`;}}/>
                <div className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-[1.5px]"
                  style={{background:c.online?"#4ade80":"#fbbf24",borderColor:"#0f1117"}}/>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <p className="text-[11px] font-bold text-white truncate">{c.name}</p>
                  {activeCard===i&&<CheckCircle className="h-3 w-3 shrink-0" style={{color:c.color}}/>}
                </div>
                <div className="flex items-center gap-1 mt-0.5">
                  <span className="text-[8px] rounded-full px-1.5 py-0.5 font-semibold" style={{background:`${c.color}20`,color:c.color}}>{c.role}</span>
                  <Star className="h-2 w-2 fill-amber-400 text-amber-400"/>
                  <span className="text-[9px] font-bold text-white">{c.rating}</span>
                  <span className="text-[9px] ml-auto font-bold" style={{color:c.color}}>{c.price} so'm</span>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        <div className="flex justify-around py-2 px-4"
          ...
          {[{icon:"🏠",label:"Bosh"},{icon:"🔍",label:"Qidirish"},{icon:"💬",label:"Chat"},{icon:"👤",label:"Profil"}].map((nav,i)=>(
            <div key={i} className="flex flex-col items-center gap-0.5"
              style={{color:i===0?"#818cf8":"rgba(255,255,255,0.28)"}}>
              <span style={{fontSize:13}}>{nav.icon}</span>
              <span style={{fontSize:7,fontWeight:600}}>{nav.label}</span>
              {i===0&&<div className="h-0.5 w-3 rounded-full bg-indigo-400 mt-0.5"/>}
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-center pb-2.5 pt-1">
          <div className="h-1 w-20 rounded-full" style={{background:"rgba(255,255,255,0.15)"}}/>
        </div>
      </motion.div>

      {/* Floating badges — compact */}
      <motion.div initial={{opacity:0,x:16}} animate={{opacity:1,x:0}} transition={{delay:0.9,type:"spring"}}
        className="absolute -right-10 top-12 z-10 rounded-xl p-2.5 shadow-2xl"
        style={{background:"rgba(18,20,32,0.96)",border:"1px solid rgba(255,255,255,0.07)",backdropFilter:"blur(16px)"}}>
        <div className="flex items-center gap-2">
          <div className="h-7 w-7 rounded-lg bg-green-500/15 flex items-center justify-center shrink-0">
            <CheckCircle className="h-3.5 w-3.5 text-green-500"/>
          </div>
          <div>
            <p className="text-[11px] font-bold text-white leading-none">Tasdiqlandi!</p>
            <p className="text-[9px] mt-0.5" style={{color:"rgba(255,255,255,0.4)"}}>Buyurtma #2847</p>
          </div>
        </div>
      </motion.div>

      <motion.div initial={{opacity:0,x:-16}} animate={{opacity:1,x:0}} transition={{delay:1.2,type:"spring"}}
        className="absolute -left-10 bottom-28 z-10 rounded-xl p-2.5 shadow-2xl"
        style={{background:"rgba(18,20,32,0.96)",border:"1px solid rgba(255,255,255,0.07)",backdropFilter:"blur(16px)"}}>
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-[11px] font-bold text-white leading-none">4.9 reyting</p>
            <p className="text-[9px] mt-0.5" style={{color:"rgba(255,255,255,0.4)"}}>12 500+ usta</p>
          </div>
        </div>
      </motion.div>

      <motion.div initial={{opacity:0,scale:0}} animate={{opacity:1,scale:1}} transition={{delay:1.5,type:"spring"}}
        className="absolute -right-6 bottom-36 z-10 rounded-lg px-2.5 py-1.5 shadow-lg"
        style={{background:"linear-gradient(135deg,#10b981,#059669)"}}>
        <div className="flex items-center gap-1">
          <motion.div animate={{scale:[1,1.3,1]}} transition={{duration:1,repeat:Infinity}}
            className="h-1.5 w-1.5 rounded-full bg-white"/>
          <p className="text-[10px] font-bold text-white">Yangi buyurtma!</p>
        </div>
      </motion.div>
    </div>
  );
...
const HeroSection = () => {
  ...
  const handleSearch = () => {
    const el = document.getElementById(query.trim() ? "mutaxassislar" : "xizmatlar");
...
<<<<<<< HEAD
        <div className="absolute inset-0 -z-10">
          ...
          <div className="absolute inset-0 opacity-[0.015]"
            style={{backgroundImage:"linear-gradient(rgba(99,102,241,0.8) 1px,transparent 1px),linear-gradient(90deg,rgba(99,102,241,0.8) 1px,transparent 1px)",backgroundSize:"80px 80px"}}/>
        </div>
            ...
            <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{duration:0.6}}>
              <h1 className="text-4xl font-bold leading-tight text-foreground md:text-5xl xl:text-6xl">
                ...
                <span className="relative inline-block">
                  <span className="bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">{t("hero_title_2")}</span>
              ...
              <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.3}}
                className="mt-8 flex items-center gap-2 rounded-2xl border border-border bg-card/80 p-2 backdrop-blur-sm shadow-lg focus-within:ring-2 ring-primary/30">
                <Search className="ml-2 h-5 w-5 shrink-0 text-muted-foreground"/>
                <input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleSearch()}
                  placeholder={t("search_placeholder")}
                  className="flex-1 bg-transparent text-sm outline-none text-foreground placeholder:text-muted-foreground"/>
                <Button onClick={handleSearch} size="sm" className="rounded-xl px-5">
              ...
              <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.5}}
                ...
                {quickTags.map(t=>(
                  <motion.button key={t.label} whileHover={{scale:1.05}} whileTap={{scale:0.95}}
                    onClick={()=>{setQuery(t.label);setTimeout(handleSearch,50);}}
                    className="flex items-center gap-1.5 rounded-full border border-border bg-card/60 px-3.5 py-1.5 text-xs font-medium text-foreground hover:border-primary hover:text-primary transition-all">
              ...
              <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.45}}
                ...
                <motion.div whileHover={{scale:1.03}} whileTap={{scale:0.97}}>
                  <Button size="lg" className="rounded-2xl px-8 shadow-xl" onClick={()=>setStartOpen(true)}>
                    <Zap className="mr-2 h-4 w-4"/> Hoziroq boshlang
                  </Button>
                </motion.div>
                <motion.div whileHover={{scale:1.03}} whileTap={{scale:0.97}}>
                  <Button size="lg" variant="outline" className="rounded-2xl px-8" onClick={()=>navigate("/chat")}>
                    <Play className="mr-2 h-4 w-4"/> Chat ochish
                  </Button>
                </motion.div>
              </motion.div>
              ...
              <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.6}}
                ...
                {[{icon:Shield,text:"Kafolatlangan"},{icon:Zap,text:"5 daqiqada"},{icon:Star,text:"4.9 reyting"},{icon:Award,text:"Tasdiqlangan"}].map(f=>(
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {stats.map((s,i)=>(
                <motion.div key={s.label} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}}
                  transition={{delay:0.6+i*0.08}} whileHover={{scale:1.04,y:-4}}
                  className="group rounded-2xl border bg-card/80 backdrop-blur-sm p-5 text-center shadow-sm hover:shadow-lg hover:border-primary/30 transition-all">
          ...
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{opacity:0,scale:0.9,y:30}} animate={{opacity:1,scale:1,y:0}}
              ...
              <div className="relative bg-gradient-to-br from-primary via-primary/80 to-accent p-8 text-center text-primary-foreground overflow-hidden">
                ...
                <button onClick={()=>setStartOpen(false)} className="absolute right-4 top-4 rounded-full bg-white/20 p-2 hover:bg-white/30 transition"><X className="h-4 w-4"/></button>
...
<<<<<<< HEAD
              <div className="p-5 grid gap-3">
                {[
                  ...
                  <motion.button key={opt.title} whileHover={{scale:1.02}} whileTap={{scale:0.98}} onClick={opt.action}
                    className={`group flex items-center gap-4 rounded-2xl border-2 p-4 text-left transition-all ${opt.color}`}>
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-secondary text-2xl group-hover:scale-110 transition-transform">{opt.emoji}</div>
=======
